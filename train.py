...
# data
parser.add_argument('--data_dir',default='./target/image',type=str, help='training dir path')
...
parser.add_argument('--total_epoch', default=5, type=int, help='total training epoch')
# backbone
...
image_datasets = {}
image_datasets['train'] = ours_datasets.OursDataset(os.path.join(data_dir, 'train' + train_all),
                                                    data_transforms['train'])
image_datasets['val'] = ours_datasets.OursDataset(os.path.join(data_dir, 'val'),
                                                  data_transforms['val'])
    ...
    for epoch in range(num_epochs):
        ...
        for phase in ['train', 'val']:
            ...
            for data in dataloaders[phase]:
                # get the image
                images, labels, texts = data
                labels = torch.from_numpy(np.array([x for x in labels]))
                ...
                if use_gpu:
                    ...
                    texts = Variable(texts.cuda().detach())
                else:
                    images, labels, texts = Variable(images), Variable(labels), Variable(texts)
        ...
        print()
        save_network(model, '{} Loss_{:.4f} Acc_{:.4f}.pkl'.format(
                phase, epoch_loss, epoch_acc))
    time_elapsed = time.time() - since
    print('Training complete in {:.0f}m {:.0f}s'.format(
        time_elapsed // 60, time_elapsed % 60))
...
model = Model(len(class_names), opt.droprate, stride=2, circle = False, ibn=False, linear_num=opt.linear_num)
